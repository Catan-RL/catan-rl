...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of processes for forward search policy.",
    )
    parser.add_argument(
        "--max-init-actions",
        type=int,
        default=10,
        help="default maximum actions to be considered by forward search.",
    )
    parser.add_argument(
        "--thinking-time",
        type=float,
        default=10,
        help="default thinking time (seconds) per decision for forward search policy.",
    )
    parser.add_argument(
        "--max-depth",
        type=int,
        default=15,
        help="number of decisions forward search simulations go.",
    )
    parser.add_argument(
        "--dont-consider-all-opening-moves",
        action="store_true",
        default=False,
        help="by default in initial placement phase, forward search considers all possible moves. requires more thinking time for these decisions.",
    )
    parser.add_argument(
        "--trades-on",
        action="store_true",
        default=False,
        help="turn trades on (agents are not generally very good at trading which can be annoying...)",
    )
    ...
    for i, policy in enumerate(
        [args.policy1, args.policy2, args.policy3, args.policy4]
    ):
        if policy == "human":
            ...
            policy_state_dict = torch.load(
                "RL/results/default_after_update_" + str(value) + ".pt",
                map_location=device,
            )
