...
class SubProcGameManager(object):
    ...
    def __init__(self, game_manager_fns, start_method=None):
        ...
        for work_remote, remote, game_manager_fn in zip(
            self.work_remotes, self.remotes, game_manager_fns
        ):
    ...
    def update_policy(self, state_dict, process_id=None, policy_id=0):
        if process_id is None:
            for remote in self.remotes:
                remote.send(
                    ("update_policy", (CloudpickleWrapper(state_dict), policy_id))
                )
            results = [remote.recv() for remote in self.remotes]
        else:
            self.remotes[process_id].send(
                ("update_policy", (CloudpickleWrapper(state_dict), policy_id))
            )
