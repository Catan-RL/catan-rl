...
class RecurrentResourceActionHead(nn.Module):
    ...
    def __init__(
        ...
        self.distribution = Categorical(
            num_inputs=dist_input_size, num_outputs=available_resources_dim
        )
    ...
    def forward(
        ...
        output = torch.zeros(
            main_inputs.size(0),
            self.available_resources_dim,
            dtype=torch.float32,
            device=self.dummy_param.device,
        )
        log_prob_sum = 0
        entropy_sum = 0
        current_resources = custom_inputs[
            "current_resources"
        ]  # in one-hot encoded form where first entry is 0 and represents stop/no res
        if self.mask_based_on_curr_res:
            mask = (current_resources > 0).float()
        else:
            mask = torch.ones_like(
                current_resources, dtype=torch.float32, device=self.dummy_param.device
            )
        res_sum = torch.sum(current_resources, dim=-1)
        zero_res_mask = res_sum == 0
        mask[:, 0] = 0.0
        mask[zero_res_mask, 0] = (
            1.0  # allow no res as first res if sum of current res = 0 (logits will be masked out anyway but leads to error o/w)
        )
        if count is None:
        ...
        for i in range(count):
            ...
            current_resources = torch.clamp(
                current_resources - one_hot_action, 0, math.inf
            )
        ...
        for prev_head_ind, head_type_mask in log_prob_masks.items():
            head_prob_mask = head_type_mask[
                prev_head_action_outs[prev_head_ind].squeeze()
            ].view(-1, 1)
            log_prob_mask *= head_prob_mask
