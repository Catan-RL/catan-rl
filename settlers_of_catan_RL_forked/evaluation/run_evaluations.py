...
if __name__ == "__main__":
    ...
    policy_id_idxs = np.arange(
        first_id_idx, len(policy_file_ids), args.evaluate_every_nth_policy
    )
    policies_to_evaluate_ids = policy_file_ids[
        first_id_idx :: args.evaluate_every_nth_policy
    ]

    results = {}

    eval_manager_fns = [make_evaluation_manager() for _ in range(NUM_PROCESSES)]
    ...
    for i, player_id in enumerate(policies_to_evaluate_ids):
        ...
        for entry in type_log_prob_tuples:
            ...
            type_prob_dict[entry[0]] = (
                type_prob_sum[entry[0]] / type_prob_count[entry[0]]
            )
        ...
        print(
            "{} games for policy after {} updates completed in {} seconds!".format(
                args.evaluation_games_per_policy, player_id, time.time() - t1
            )
        )

        print(
            "-----------------    WIN FRAC: {}".format(results[player_id]["win_frac"])
        )
        print(
            "-----------------    AVG GAME LENGTH: {}".format(
                results[player_id]["avg_game_length"]
            )
        )
        print(
            "-----------------    AVG POLICY DECISIONS: {}".format(
                results[player_id]["avg_pol_decisions"]
            )
        )
        print(
            "-----------------    AVG VICTORY POINTS: {}".format(
                results[player_id]["avg_vps"]
            )
        )
        print(
            "-----------------    AVG ENTROPY: {}".format(
                results[player_id]["avg_entropy"]
            )
        )
        print(
            "-----------------    DRAW FRACTION: {}".format(
                results[player_id]["draw_frac"]
            )
        )
        print("action types:")
