...
parser.add_argument(
    "--consider-all-moves-for-opening-placements", action="store_true", default=False
)
...
if __name__ == "__main__":
    policy_state_dict = torch.load(
        Path("RL", "results", args.base_policy_file), map_location="cpu"
    )
    ...
    if args.other_policies != "":
        ...
        other_policy_state_dicts = [
            torch.load(Path("RL", "results", other_policies[i]), map_location="cpu")
            for i in range(3)
        ]
    else:
        other_policy_state_dicts = [
    ...
    forward_search_policy = ForwardSearchPolicy(
        policy_state_dict,
        default_sample_actions,
        args.max_init_actions,
        args.max_depth,
        args.max_thinking_time,
        gamma=args.gamma,
    ...
    for i in range(args.num_games):
        winner, victory_points, total_steps, policy_decisions, _, action_types, _, _ = (
            evaluation_manager.run_evaluation_game()
        )
        ...
        print(
            "Game {} finished. Winner: {}. Fraction of games won by forward search so far: {}".format(
        ...
        torch.save(
            (
                ...
                victory_points_all,
                forward_policy_decisions_all,
                sorted(dict(action_types_all).items()),
            ),
