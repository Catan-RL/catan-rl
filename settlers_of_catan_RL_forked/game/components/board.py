...
class Board(object):
    def __init__(
        ...
        if fixed_number_order is not None:
            assert np.array_equal(
                [
                    fixed_number_order.count(n) == self.DEFAULT_NUMBER_ORDER.count(n)
                    for n in range(2, 13)
                ]
            )
    ...
    def reset(self):
        ...
        for i in range(19):
            if self.tiles[self.NUMBER_PLACEMENT_INDS[i]].terrain == Terrain.Desert:
                ...
                self.value_to_tiles[number_order[num_ind]] = self.value_to_tiles.get(
                    number_order[num_ind], []
        ...
        for tile_ind in range(19):
            for corner_location in self.tiles[tile_ind].corners.keys():
                ...
                if prev_tile_ind is None:
                    self.tiles[tile_ind].corners[corner_location] = self.corners[
                        corner_ind
                    ]
                    corner_ind += 1
                else:
        ...
        for corner in self.corners:
            for n_corner in corner.corner_neighbours:
                if n_corner[0] is not None:
                    ...
                    self.corner_egde_identification_map[corner.id, n_corner[0].id] = (
                        n_corner[1].id
                    )
