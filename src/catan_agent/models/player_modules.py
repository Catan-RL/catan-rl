...
class CurrentPlayerModule(nn.Module):
    def __init__(
        self, main_input_dim, dev_card_embed_dim, dev_card_model_dim, proj_dev_card_dim
    ):
        ...
        self.proj_hidden_dev_card = init_(
            nn.Linear(dev_card_model_dim, proj_dev_card_dim)
        )
        self.proj_played_dev_card = init_(
            nn.Linear(dev_card_model_dim, proj_dev_card_dim)
        )
    ...
    def forward(
        ...
        if isinstance(hidden_dev_cards, list):
            hidden_dev_cards_lengths = [
                len(hidden_cards) for hidden_cards in hidden_dev_cards
            ]
            padded_hidden_dev_cards = pad_sequence(
                hidden_dev_cards, batch_first=True
            ).long()
        else:
            hidden_dev_cards_lengths = (
                (hidden_dev_cards.shape[-1] - (hidden_dev_cards == 0).sum(dim=-1))
                .cpu()
                .data.numpy()
            )
        ...
        hidden_dev_card_masks = (
            hidden_dev_card_masks.squeeze(1).transpose(-1, -2).long()
        )
        hidden_dev_representations[
            hidden_dev_card_masks.repeat(1, 1, hidden_dev_representations.shape[-1])
            == 0
        ] = 0.0
        ...
        hidden_dev_out = self.relu(
            self.norm_2(self.proj_hidden_dev_card(hidden_dev_out))
        )

        if isinstance(played_dev_cards, list):
            played_dev_card_lengths = [
                len(played_cards) for played_cards in played_dev_cards
            ]
            padded_played_dev_cards = pad_sequence(
                played_dev_cards, batch_first=True
            ).long()
        else:
            played_dev_card_lengths = (
                (played_dev_cards.shape[-1] - (played_dev_cards == 0).sum(dim=-1))
                .cpu()
                .data.numpy()
            )
        ...
        played_dev_card_masks = (
            played_dev_card_masks.squeeze(1).transpose(-1, -2).long()
        )
        played_dev_representations[
            played_dev_card_masks.repeat(1, 1, played_dev_representations.shape[-1])
            == 0
        ] = 0.0
        ...
        played_dev_out = self.relu(
            self.norm_3(self.proj_played_dev_card(played_dev_out))
        )
...
class OtherPlayersModule(nn.Module):
    ...
    def forward(
        self, main_input, played_dev_cards, dev_card_embedding, played_card_mha
    ):
        if isinstance(played_dev_cards, list):
            played_dev_card_lengths = [
                len(played_cards) for played_cards in played_dev_cards
        ...
        played_dev_out = played_dev_representations.sum(dim=1)
        played_dev_out = self.relu(
            self.norm_2(self.proj_played_dev_card(played_dev_out))
        )
