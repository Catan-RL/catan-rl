...
from catan_agent.models.multi_headed_attention import MultiHeadedAttention
from catan_agent.models.player_modules import CurrentPlayerModule, OtherPlayersModule
from catan_agent.models.tile_encoder import TileEncoder
...
class ObservationModule(nn.Module):
    def __init__(
        ...
        self.final_layer = init_(
            nn.Linear(19 * proj_tile_dim + 4 * 128, observation_out_dim)
        )
        self.relu = nn.ReLU()
    ...
    def forward(self, obs_dict):
        ...
        other_player_outputs = [
            self.other_players_module(
                ...
                obs_dict[o_player + "_player_played_dev"],
                self.dev_card_embedding,
                self.played_card_mha,
            )
        ...
        final_input = torch.cat(
            (tile_encodings, current_player_output, *other_player_outputs), dim=-1
        )
