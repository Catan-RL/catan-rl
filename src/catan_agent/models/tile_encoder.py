...
from catan_agent.models.utils import get_clones
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, rep_batch):
        encoder_self_attention = lambda rb: self.multi_headed_attention(
            query=rb, key=rb, value=rb, mask=None
        )
...
class TileEncoder(nn.Module):
    def __init__(
        self, tile_in_dim, model_dimension, num_heads, num_layers, out_proj_dim
    ):
    ...
    def forward(self, tile_representations):
        ...
        tile_representations = self.relu(
            self.norm_2(self.first_layer(tile_representations))
        )
