...
from catan_agent.models.utils import ValueFunctionNormaliser
...
class SettlersAgentPolicy(nn.Module):
    def __init__(
        ...
        observation_module,
        action_head_module,
        observation_out_dim=128,
        include_lstm=False,
        lstm_size=256,
        value_mlp_sizes=[256, 128],
        value_normalisation=True,
    ):
        ...
        if include_lstm:
            self.lstm = nn.LSTM(
                ...
                input_size=observation_out_dim,
                batch_first=False,
            )
    ...
    def base(self, obs_dict, hidden_states, done_masks):
        ...
        if self.include_lstm:
            lstm_output, hidden_states = self._forward_lstm(
                observation_out, hidden_states, done_masks
            )
        ...
        value = self.value_out(
            self.relu(
                self.v_norm_2(
                    self.value_network_fc_2(
                        self.relu(
                            self.v_norm_1(self.value_network_fc_1(observation_out))
                        )
                    )
                )
            )
    ...
    def act(
        ...
        log_specific_action_output=False,
    ):
        custom_inputs = {
            "current_resources": obs_dict["current_resources"]
        }

        value, main_out, hidden_states = self.base(
            obs_dict, hidden_states, nonterminal_masks
        )
        ...
        actions, action_log_probs, entropy, log_output = self.action_head_module(
            ...
            condition_on_action_type=condition_on_action_type,
    ...
    def evaluate_actions(
        self, obs_dict, hidden_states, nonterminal_masks, actions, action_masks
    ):
        custom_inputs = {
            "current_resources": obs_dict["current_resources"]
        }
    ...
    def _forward_lstm(self, x, hidden_states, nonterminal_masks):
        ...
        if x.size(0) == hxs.size(0):
            x, hxs = self.lstm(
                x.unsqueeze(0),
                (
                    (hxs * nonterminal_masks).unsqueeze(0),
            ...
            has_zeros = (
                (nonterminal_masks[1:] == 0.0).any(dim=-1).nonzero().squeeze().cpu()
            )
            ...
            for i in range(len(has_zeros) - 1):
                ...
                rnn_scores, hidden_states = self.lstm(
                    x[start_idx:end_idx],
                    (
                        (hxs * nonterminal_masks[start_idx].view(1, -1, 1)),
