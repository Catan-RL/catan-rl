...
class MultiHeadedAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask):
        ...
        if mask is not None:
            scores.masked_fill_(
                mask == torch.tensor(False, device=self.dummy_param.device),
                float("-inf"),
            )
    ...
    def forward(self, query, key, value, mask):
        ...
        query, key, value = [
            net(x)
            .view(batch_size, -1, self.num_heads, self.head_dimension)
            .transpose(1, 2)
        ...
        intermediate_reps = intermediate_reps.transpose(1, 2).reshape(
