...
def worker(
    ...
    while True:
        ...
        if cmd == "initialise_policy":
            ...
            initial_state_to_load, starting_hidden_states, starting_observation = (
                data.var
            )
            remote.send(True)
        elif cmd == "set_initial_actions":
            first_actions_under_consideration, player_hidden_states_after_first_ac = (
                data.var
            )
            remote.send(True)
            ...
            pred_val = run_simulation_forward(
                env,
                policy,
...
def gae(values, rewards, gamma, done):
    ...
    for step in reversed(range(num_steps)):
        delta = (
            rewards_for_gae[step]
            + gamma * values_for_gae[step + 1]
            - values_for_gae[step]
        )
