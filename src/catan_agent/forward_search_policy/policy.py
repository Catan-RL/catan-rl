...
from catan_env.game.enums import PlayerId
from catan_agent.forward_search_policy.utils import MovingAvgCalculator
from catan_agent.forward_search_policy.worker import worker
...
class ForwardSearchPolicy(object):
    def __init__(
        ...
        gamma=0.999,
        num_subprocesses=11,
        subprocess_start_method=None,
        player_id=None,
        zero_opponent_hidden_states=True,
        consider_all_moves_for_opening_placement=False,
        dont_propose_devcards=False,
        lstm_size=256,
    ):
        ...
        self.consider_all_moves_for_opening_placement = (
            consider_all_moves_for_opening_placement
        )
        ...
        self.remotes, self.work_remotes = zip(
            *[ctx.Pipe() for _ in range(num_subprocesses)]
        )
        self.processes = []
        ...
        for remote in self.remotes:
            remote.send(
                (
                    "initialise_policy",
                    CloudpickleWrapper(
                        (self.base_policy.state_dict(), self.player_id, self.gamma)
                    ),
                )
            )
    ...
    def act(
        ...
        curr_env_state,
        curr_action_masks,
        initial_settlement=False,
        decision_no=0,
    ):
        self.proposed_actions, self.player_next_hidden_states = self.sample_actions_fn(
            curr_obs,
            curr_hidden_states[self.player_id],
            curr_action_masks,
            self.base_policy,
            self.max_init_actions,
            ...
            dont_propose_devcards=self.dont_propose_devcards,
        )
        ...
        thinking_time = self.max_thinking_time * (
            len(self.proposed_actions) / self.max_init_actions
        )
        ...
        self.num_simulations_finished_each_action = np.zeros(
            (len(self.proposed_actions),)
        )
        self.num_simulations_started_each_action = np.zeros(
            (len(self.proposed_actions),)
        )
        ...
        for remote in self.remotes:
            remote.send(
                (
                    "set_start_states",
                    CloudpickleWrapper((curr_env_state, curr_hidden_states, curr_obs)),
                )
            )
        confirmation = [remote.recv() for remote in self.remotes]
        for remote in self.remotes:
            remote.send(
                (
                    "set_initial_actions",
                    CloudpickleWrapper(
                        (self.proposed_actions, self.player_next_hidden_states)
                    ),
                )
            )
        confirmation = [remote.recv() for remote in self.remotes]
        ...
        while elapsed_time < thinking_time:
            ...
            while len(self.workers_ready_to_simulate) > 0:
                ...
                self.remotes[worker_id].send(
                    ("run_simulation", (action_id, self.max_depth))
                )
        ...
        return (
            self.proposed_actions[best_action_id],
            self.player_next_hidden_states[best_action_id],
        )
    ...
    def _select_action(self, explore=True):
        ...
        for i in range(len(self.proposed_actions)):
            exploit_score = self.exploit_scores[i] / (
                self.num_simulations_finished_each_action[i] + 1e-5
            )
            explore_score = math.sqrt(
                2.0
                * math.log(self.num_simulations_finished + 2)
                / (
                    self.num_simulations_finished_each_action[i]
                    + self.num_simulations_started_each_action[i]
                    + 1e-10
                )
            )
            if explore:
    ...
    def obs_to_torch(self, obs):
        for key in self.standard_obs_keys:
            obs[key] = torch.tensor(
                obs[key], dtype=torch.float32, device=self.dummy_param.device
            )
        ...
        for key in self.list_int_obs_keys:
            if isinstance(obs[key][0], list):
                for k in range(len(obs[key])):
                    obs[key][k] = torch.tensor(
                        obs[key][k], dtype=torch.long, device=self.dummy_param.device
                    )
            else:
                obs[key] = [
                    torch.tensor(
                        obs[key], dtype=torch.long, device=self.dummy_param.device
                    )
                ]
    ...
    def act_masks_to_torch(self, masks):
        for z in range(len(masks)):
            masks[z] = torch.tensor(
                masks[z], dtype=torch.float32, device=self.dummy_param.device
            ).unsqueeze(0)
