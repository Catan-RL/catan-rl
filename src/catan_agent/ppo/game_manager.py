...
from catan_env.wrapper import EnvWrapper
...
class GamesAndPoliciesManager(object):
    ...
    def initialise(self):
        ...
        for i in range(self.num_envs):
            ...
            self.active_player_ids.append(
                order[0]
            )  # active player controlled by policy[0]
            policy_map = {}
            for j in range(4):
    ...
    def reset(self):
        ...
        for i in range(self.num_envs):
            ...
            if players_go == self.active_player_ids[i]:
                ...
                self.active_hidden_states[i].append(
                    (
            ...
            for player_id in [
                ...
                self.current_hidden_states[i][player_id] = (
                    torch.zeros(1, self.policies[0].lstm_size, device=self.device),
                    torch.zeros(1, self.policies[0].lstm_size, device=self.device),
                )
        ...
        with torch.no_grad():
            for env_num in range(self.num_envs):
                ...
                while len(self.observations[env_num]) < self.num_steps + 1:
                    ...
                    action_masks = self.policies[0].act_masks_to_torch(
                        self.envs[env_num].get_action_masks()
                    )

                    _, actions, action_log_probs, hidden_states = self.policy_maps[
                        env_num
                    ][players_go].act(obs, hidden_states, terminal_mask, action_masks)

                    self.current_hidden_states[env_num][players_go] = hidden_states

                    obs, reward, done, _ = self.envs[env_num].step(
                        self.policies[0].torch_act_to_np(actions)
                    )
                    ...
                    terminal_mask = 1.0 - torch.tensor(
                    ...
                    if (
                        n_players_go == self.active_player_ids[env_num]
                        and len(self.actions[env_num]) > 0
                    ):
                        if done_since_prev_turn[env_num] == False:
                            self.rewards[env_num].append(
                                rewards[self.active_player_ids[env_num]]
                            )
                    ...
                    if done:
                        ...
                        for player_id in [
                            ...
                            self.current_hidden_states[env_num][player_id] = (
                                torch.zeros(
                                    1, self.policies[0].lstm_size, device=self.device
                                ),
                                torch.zeros(
                                    1, self.policies[0].lstm_size, device=self.device
                                ),
                    ...
                    if n_players_go == self.active_player_ids[env_num]:
                        ...
                        self.active_hidden_states[env_num].append(
                            self.current_hidden_states[env_num][n_players_go]
                        )
                    else:
                        if done:
    ...
    def _after_rollouts(self):
        for env_num in range(self.num_envs):
            ...
            self.active_hidden_states[env_num] = [
                self.active_hidden_states[env_num][-1]
            ]
