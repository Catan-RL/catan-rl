...
class PPO:
    def __init__(self, actor_critic, args):
        ...
        self.optimiser = torch.optim.Adam(
            actor_critic.parameters(), lr=args.lr, eps=args.eps
        )
    ...
    def update(self, rollout_storage):
        ...
        for e in range(self.ppo_epoch):
            ...
            if self.actor_critic.include_lstm:
                total_batch_size = (
                    rollout_storage.num_parallel * rollout_storage.num_steps
                )
                data_generator = rollout_storage.generator_lstm(
            ...
            for sample in data_generator:
                ...
                if self.actor_critic.use_value_normalisation:
                    value_preds_batch = self.actor_critic.value_normaliser.normalise(
                        value_preds_batch
                    )
                    returns_batch = self.actor_critic.value_normaliser.normalise(
                        returns_batch
                    )

                values, action_log_probs, entropy, _ = (
