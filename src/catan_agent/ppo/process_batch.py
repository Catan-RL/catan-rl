...
from catan_agent.ppo.utils import _flatten_helper, _flatten_helper_reshape
...
class BatchProcessor(object):
    def __init__(
        ...
        obs_keys=OBS_KEYS,
        obs_type=OBS_TYPES,
        type_conditional_masks=TYPE_CONDITIONAL_MASKS,
        num_action_heads=12,
        device="cuda",
        stored_device="cpu",
    ):
    ...
    def process_rollouts(self, rollouts):
        ...
        for i, key in enumerate(self.obs_keys):
            if self.obs_type[i] == "list":
                p1 = [
                    [obs[k][t][key][0] for k in range(self.num_parallel)]
                    for t in range(self.num_steps + 1)
                ]
                p2 = list(itertools.chain.from_iterable(p1))
                p3 = pad_sequence(p2, batch_first=True)
                self.obs_dict[key] = p3.view(
                    self.num_steps + 1, self.num_parallel, p3.shape[1]
                ).to(self.stored_device)
            else:
                self.obs_dict[key] = torch.stack(
                    [
                        torch.cat(
                            [obs[k][t][key] for k in range(self.num_parallel)], dim=0
                        )
                        for t in range(self.num_steps + 1)
                    ]
                ).to(self.stored_device)
        ...
        self.hidden_states = (
            torch.stack(
                [
                    torch.cat(
                        [hidden_states[k][t][0] for k in range(self.num_parallel)],
                        dim=0,
                    )
                    for t in range(self.num_steps + 1)
                ]
            ...
            torch.stack(
                [
                    torch.cat(
                        [hidden_states[k][t][1] for k in range(self.num_parallel)],
                        dim=0,
                    )
                    for t in range(self.num_steps + 1)
                ]
        ...
        self.action_log_probs = torch.stack(
            [
                torch.cat(
                    [action_log_probs[k][t] for k in range(self.num_parallel)], dim=0
                )
                for t in range(self.num_steps)
            ]
        ).to(self.stored_device)
        ...
        self.masks = torch.stack(
            [
                torch.cat(
                    [
                        torch.tensor(masks[k][t], dtype=torch.float32).view(1, 1)
                        for k in range(self.num_parallel)
                    ],
                    dim=0,
                )
    ...
    def compute_advantages_alt(self, actor_critic, max_processes_at_once=10):
        ...
        self.values = torch.zeros(self.num_steps + 1, self.num_parallel, 1).to(
            self.stored_device
        )
        ...
        for i in range(len(start_inds)):
            ...
            if actor_critic.include_lstm:
                recurrent_hidden_states_in = (
                    _flatten_helper_reshape(
                        self.num_steps + 1,
                        num_proc,
                        self.hidden_states[0][:, start_inds[i] : end_inds[i], ...],
                    ).to(self.device),
                    _flatten_helper_reshape(
                        self.num_steps + 1,
                        num_proc,
                        self.hidden_states[1][:, start_inds[i] : end_inds[i], ...],
                    ).to(self.device),
                )
            else:
            ...
            for key in self.obs_keys:
                obs_dict_in[key] = _flatten_helper_reshape(
                    self.num_steps + 1,
                    num_proc,
                    self.obs_dict[key][:, start_inds[i] : end_inds[i], ...],
                ).to(self.device)

            masks_in = _flatten_helper_reshape(
                self.num_steps + 1,
                num_proc,
                self.masks[:, start_inds[i] : end_inds[i], ...],
            ).to(self.device)

            self.values[:, start_inds[i] : end_inds[i], :] = (
                actor_critic.get_value(
                    obs_dict_in, recurrent_hidden_states_in, masks_in
                )
                .reshape(self.num_steps + 1, num_proc, -1)
                .to(self.stored_device)
            )
        ...
        for step in reversed(range(self.num_steps)):
            delta = (
                self.rewards[step]
                + self.args.gamma * self.values[step + 1] * self.masks[step + 1]
                - self.values[step]
            )
            gae = (
                delta
                + self.args.gamma * self.args.gae_lambda * self.masks[step + 1] * gae
            )
    ...
    def generator_standard(self, num_mini_batch):
        ...
        for indices in sampler:
            ...
            for key in self.obs_keys:
                obs_dict_batch[key] = (
                    self.obs_dict[key][:-1]
                    .view(-1, *self.obs_dict[key].size()[2:])[indices]
                    .to(self.device)
                )
            ...
            for i in range(self.num_action_heads):
                if i in self.type_conditional_masks:
                    action_masks_batch[i] = (
                        self.action_masks[i]
                        .view(
                            self.action_masks[i].size()[0],
                            -1,
                            *self.action_masks[i].size()[3:],
                        )[:, indices, ...]
                        .to(self.device)
                    )
                else:
                    action_masks_batch[i] = (
                        self.action_masks[i]
                        .view(-1, *self.action_masks[i].size()[2:])[indices]
                        .to(self.device)
                    )
                actions_batch[i] = (
                    self.actions[i]
                    .view(-1, *self.actions[i].size()[2:])[indices]
                    .to(self.device)
                )
            ...
            old_action_log_probs_batch = self.action_log_probs.view(-1, 1)[indices].to(
                self.device
            )
            adv_targets = self.advantages.view(-1, 1)[indices].to(self.device)

            yield obs_dict_batch, recurrent_hidden_states_batch, actions_batch, action_masks_batch, value_preds_batch, returns_batch, masks_batch, old_action_log_probs_batch, adv_targets
    ...
    def generator_lstm(self, num_mini_batch, total_batch_size, truncated_seq_len):
        ...
        num_sequences_per_minibatch = (
            total_batch_size // num_mini_batch // truncated_seq_len
        )
        ...
        for start_ind in range(0, len(inds_permutation), num_sequences_per_minibatch):
            ...
            for offset in range(num_sequences_per_minibatch):
                ...
                recurrent_hidden_batch[0].append(
                    self.hidden_states[0][t_inds[0] : t_inds[1], p_inds[0], ...]
                )
                recurrent_hidden_batch[1].append(
                    self.hidden_states[1][t_inds[0] : t_inds[1], p_inds[0], ...]
                )

                for i in range(self.num_action_heads):
                    actions_batch[i].append(self.actions[i][t_inds, p_inds, ...])
                    if i in self.type_conditional_masks:
                        action_masks_batch[i].append(
                            self.action_masks[i][:, t_inds, p_inds, ...]
                        )
                    else:
                        action_masks_batch[i].append(
                            self.action_masks[i][t_inds, p_inds, ...]
            ...
            recurrent_hidden_batch[0] = (
                torch.stack(recurrent_hidden_batch[0], 1).view(N, -1).to(self.device)
            )
            recurrent_hidden_batch[1] = (
                torch.stack(recurrent_hidden_batch[1], 1).view(N, -1).to(self.device)
            )
            ...
            # flatten
            for key in self.obs_keys:
                obs_dict_batch[key] = _flatten_helper(
                    truncated_seq_len, N, obs_dict_batch[key]
                ).to(self.device)

            for i in range(self.num_action_heads):
                actions_batch[i] = _flatten_helper(
                    truncated_seq_len, N, actions_batch[i]
                ).to(self.device)
                if i in self.type_conditional_masks:
                    num_types = action_masks_batch[i].shape[0]
                    T, N = (
                        action_masks_batch[i].shape[1],
                        action_masks_batch[i].shape[2],
                    )
                    action_masks_batch[i] = (
                        action_masks_batch[i].view(num_types, T * N, -1).to(self.device)
                    )
                else:
                    action_masks_batch[i] = _flatten_helper(
                        truncated_seq_len, N, action_masks_batch[i]
                    ).to(self.device)

            value_preds_batch = _flatten_helper(
                truncated_seq_len, N, value_preds_batch
            ).to(self.device)
            returns_batch = _flatten_helper(truncated_seq_len, N, returns_batch).to(
                self.device
            )
            masks_batch = _flatten_helper(truncated_seq_len, N, masks_batch).to(
                self.device
            )
            old_action_log_probs_batch = _flatten_helper(
                truncated_seq_len, N, old_action_log_probs_batch
            ).to(self.device)
            adv_targets = _flatten_helper(truncated_seq_len, N, adv_targets).to(
                self.device
            )

            yield obs_dict_batch, recurrent_hidden_batch, actions_batch, action_masks_batch, value_preds_batch, returns_batch, masks_batch, old_action_log_probs_batch, adv_targets
