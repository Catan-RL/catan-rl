...
class EvaluationManager(object):
    ...
    def reset(self):
        ...
        for player_id in [PlayerId.Blue, PlayerId.Red, PlayerId.Orange, PlayerId.White]:
            self.current_hidden_states[player_id] = (
    ...
    def run_evaluation_game(self):
        ...
        while done == False:
            ...
            with torch.no_grad():
                ...
                action_masks = self.policies[-1].act_masks_to_torch(
                    self.env.get_action_masks()
                )

                if (
                    self.policies[self.policy_map[players_go]].policy_type
                    == "neural_network"
                ):
                    if self.detailed_logging:
                        (
                            ...
                            entropy,
                            detailed_action_out,
                        ) = self.policies[self.policy_map[players_go]].act(
                            obs,
                            hidden_states,
                            terminal_mask,
                            action_masks,
                            deterministic=False,
                            return_entropy=True,
                            log_specific_action_output=True,
                        )
                    else:
                        (
                            value,
                    ...
                    if self.env.game.initial_settlements_placed[players_go] == 0:
                        ...
                        placing_initial_settlement = True
                    actions, hidden_states = self.policies[
                        self.policy_map[players_go]
                    ].act(
                        obs,
                        self.current_hidden_states,
                        curr_state,
                        action_masks,
                        decision_no=policy_decisions,
