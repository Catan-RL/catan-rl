...
class Edge(object):
    ...
    def can_place_road(
        self, player: PlayerId, after_second_settlement=False, second_settlement=None
    ):
        if self.road is not None:
            ...
            if after_second_settlement:
                if (
                    self.corner_1.id == second_settlement
                    or self.corner_2.id == second_settlement
                ):
                    return True
                else:
                    return False
            if (
                self.corner_1.building is not None
                and self.corner_1.building.owner == player
            ) or (
                self.corner_2.building is not None
                and self.corner_2.building.owner == player
            ):
                return True
            else:
