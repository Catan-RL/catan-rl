...
class Game(object):
    def __init__(
        self, board_config={}, interactive=False, debug_mode=False, policies=None
    ):
        ...
        for resource in resources_allocated.keys():
            if resources_allocated[resource]["total"] <= self.resource_bank[resource]:
                for player in [
                    ...
                    self.players[player].resources[resource] += resources_allocated[
                        resource
                    ][player]
                    self.resource_bank[resource] -= resources_allocated[resource][
                        player
                    ]
                    self.update_player_resource_estimates(
                        {resource: resources_allocated[resource][player]}, player
                    )
                ...
                if self.board.corners[corner].can_place_settlement(
                    ...
                    if self.initial_placement_phase:
                        if self.initial_settlements_placed[self.players_go] == 0 or (
                            self.initial_settlements_placed[self.players_go] == 1
                ...
                if self.board.edges[edge].can_place_road(player.id):
                    if self.initial_placement_phase:
                        if (
                            ...
                            self.initial_settlements_placed[self.players_go] == 2
                            and self.initial_roads_placed[self.players_go] == 1
                        ):
            ...
            if action["card"] in player.hidden_cards:
                if player.hidden_cards.count(action["card"]) > 0:
                    if player.hidden_cards.count(
                        action["card"]
                    ) == self.development_cards_bought_this_turn.count(action["card"]):
                        return (
                            False,
    ...
    def apply_action(self, action, return_message=True):
        ...
        if action["type"] == ActionTypes.PlaceSettlement:
            ...
            if edge is None:
                ...
                if self.initial_placement_phase:
                    ...
                    if first_settlements_placed < 4:
                        ...
                        first_settlements_placed == 4 and second_settlements_placed == 0
                    ):
                        pass
                    elif (
                        first_settlements_placed == 4 and second_settlements_placed < 4
                    ):
            ...
            if action["card"] == DevelopmentCard.VictoryPoint:
                ...
                card_type_text = (
                    "Monopoly card, taking " + self.resource_text[resource] + "."
                )
                num_res = {}
                ...
                card_type_text = "Year of Plenty card, collecting {} and {}.".format(
            ...
            player.visible_resources[trade_resource] = max(
                player.visible_resources[trade_resource] - action["exchange_rate"], 0
            )
            ...
            if self.longest_road["count"] > max_path_len:
                ...
                if max_p_len >= 5:
                    if tied_longest_road:
                        ...
                        self.longest_road = {"player": player, "count": max_p_len}
        ...
        for player in [PlayerId.White, PlayerId.Red, PlayerId.Blue, PlayerId.Orange]:
            if player == player_updating:
                ...
                for res, num_res in resources.items():
                    ...
                    self.players[player].opponent_max_res[stealing_player_label][
                        res
                    ] -= num_res
                    self.players[player].opponent_min_res[stealing_player_label][
                        res
                    ] -= num_res
            else:
                ...
                if player_who_stole is None:
                    ...
                    for res, num_res in resources.items():
                        ...
                        self.players[player].opponent_max_res[player_label][res] = (
                            np.clip(curr_max_est + num_res, 0, total_resources)
                        )
                        self.players[player].opponent_min_res[player_label][res] = (
                    ...
                    if player == player_who_stole:
                        ...
                        for res in [
                            ...
                            curr_max_player_est = self.players[player].opponent_max_res[
                                player_label
                            ][res]
                            curr_min_player_est = self.players[player].opponent_min_res[
                                player_label
                            ][res]
                            self.players[player].opponent_max_res[player_label][res] = (
                                np.clip(curr_max_player_est, 0, total_resources)
                            )
                            # any resource could have been stolen
                            self.players[player].opponent_min_res[player_label][res] = (
                                np.clip(curr_min_player_est - 1, 0, total_resources)
                            )
                            if curr_max_player_est > 0:
                                ...
                                self.players[player].opponent_max_res[
                                    ...
                                    curr_max_steal_est + 1, 0, total_resources_stealer
                                )
                                self.players[player].opponent_min_res[
                                    stealing_player_label
                                ][res] = np.clip(
                                    curr_min_steal_est, 0, total_resources_stealer
                                )
            ...
            if player == player_id:
                ...
                for o_player in [
                    ...
                    for o_res in [
                        ...
                        self.players[o_player].opponent_max_res[player_label][o_res] = (
                            np.clip(curr_max_est, 0, player_total_res)
                        )
                        self.players[o_player].opponent_min_res[player_label][o_res] = (
            ...
            for player_id in self.players.keys():
                if player_id != controlling_player_id:
                    player_definite_res = self.players[
                        ...
                        self.players[controlling_player_id].player_lookup[player_id]
                    ][
                        res
                    ]
