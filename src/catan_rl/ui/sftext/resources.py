    ...
    def _index(cls, path, types):
        if sys.version_info >= (3, 5):
            ...
            for img_type in types:
                for filename in glob.iglob((path + "/**/" + img_type), recursive=True):
...
class Images(Resources):
    ...
    def __init__(cls, path=".", types=["*.jpg", "*.png", "*.bmp"]):
        super().__init__(loader=pygame.image.load, path=path, types=types)
...
class Fonts(Resources):
    ...
    def __init__(cls, path=".", types=["*.ttf"]):
        super().__init__(
            loader=pygame.font.Font, path=path, types=types, weak_ref=False
        )
