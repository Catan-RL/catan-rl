...
class Display(object):
    def __init__(
            ...
            for j in range(i):
                ...
                if TL is not None and TL == j:
                    ...
                    self.scaled_tile_pos[i] = (
                        ...
                        + self.outer_hexagon_side_len * (np.sqrt(3) / 2.0),
                        self.scaled_tile_pos[j][1]
                        + (3.0 / 2.0) * self.outer_hexagon_side_len,
                    )
                    break
                    ...
                    self.tile_pos[i] = (
                        self.tile_pos[j][0] + self.hexagon_width,
                        self.tile_pos[j][1],
                    )
                    self.scaled_tile_pos[i] = (
                        self.scaled_tile_pos[j][0] + self.outer_hexagon_width,
                        self.scaled_tile_pos[j][1],
                    )
        ...
        for corner in game.board.corners:
            ...
            start_pos = [
                self.tile_pos[tile.id][0] + self.hexagon_width / 2.0,
        ...
        self.terrain_images = {
            key: pygame.transform.scale(
                pygame.image.load(os.path.join(*self.image_path, val)),
                (self.hexagon_width, self.hexagon_height),
            )
            for key, val in self.terrain_image_paths.items()
        }

        self.token_image_paths = {
            i: "value_tokens/token_" + str(i) + ".png"
        ...
        self.token_images = {
            ...
            for key, val in self.token_image_paths.items()
        }
        self.robber_image = pygame.transform.scale(
            pygame.image.load(
                os.path.join(*self.image_path, "value_tokens/token_robber.png")
            ),
            (self.token_dim, self.token_dim),
        )
        ...
        self.harbour_images = {
            ...
            for key, val in self.harbour_image_paths.items()
        }
        ...
        self.settlement_images = {
            key: pygame.transform.scale(
                pygame.image.load(os.path.join(*self.image_path, val)),
                (self.building_width, self.building_height),
            )
            for key, val in self.settlement_image_paths.items()
        }
        ...
        self.city_images = {
            ...
            for key, val in self.city_image_paths.items()
        }
        ...
        self.dice_images = {
            key: pygame.transform.scale(
                pygame.image.load(os.path.join(*self.image_path, val)),
                (self.dice_width, self.dice_height),
            )
            for key, val in self.dice_image_paths.items()
        }
        self.development_card_scale = 0.36
        self.development_card_width, self.development_card_height = int(
            368 * self.development_card_scale
        ...
        self.development_card_images = {
            key: pygame.transform.scale(
                pygame.image.load(os.path.join(*self.image_path, val)),
                (self.development_card_width, self.development_card_height),
            )
            for key, val in self.development_card_image_paths.items()
        }

        self.top_menu = pygame.image.load(
            os.path.join(*self.image_path, "menu/top_header.png")
        )
        self.action_menu = pygame.image.load(
            os.path.join(*self.image_path, "menu/action_menu.png")
        )
        self.building_cost_menu = pygame.image.load(
            os.path.join(*self.image_path, "menu/building_cost_menu.png")
        )
        self.tick_image = pygame.image.load(
            os.path.join(*self.image_path, "menu/tick.png")
        )
        ...
        self.trading_semi_circle_images = {
            key: pygame.transform.scale(
                pygame.image.load(os.path.join(*self.image_path, val)),
                (semi_circle_img_width, semi_circle_img_height),
            )
            for key, val in self.trading_semi_circle_image_paths.items()
        }
        ...
        self.resource_images = {
            key: pygame.image.load(os.path.join(*self.image_path, val))
            for key, val in self.resource_image_paths.items()
        }
    ...
    def construct_outer_board_polygon(self):
        ...
        for i in range(len(positions)):
            positions[i][0] = positions[i][0] - 1.5 * (
                self.outer_hexagon_width - self.hexagon_width
            )
            positions[i][1] = positions[i][1] - 2 * (
                self.outer_hexagon_height - self.hexagon_height
            )
    ...
    def render_board(self):
        pygame.draw.polygon(
            self.screen, pygame.Color(241, 233, 161), self.outer_board_polygon
        )
        ...
        for i, tile in enumerate(self.game.board.tiles):
            ...
            if tile.contains_robber:
                self.render_robber(
                    self.tile_pos[i][0]
                    + (self.hexagon_width / 2.0)
                    - (self.token_dim / 2.0),
                    self.tile_pos[i][1]
                    + (self.hexagon_height / 2.0)
                    + (self.token_dim / 2.0),
                )
            if tile.value != 7:
                self.render_token(
    ...
    def render_debug_screen(self):
        ...
        pygame.draw.rect(
            self.screen,
            self.road_colours[player.inverse_player_lookup["next"]],
            (1825, 100, 30, 30),
        )
        ...
        self.screen.blit(
            self.count_font.render("Next Next", False, (0, 0, 0)), (1720, 255)
        )
        pygame.draw.rect(
            self.screen,
            self.road_colours[player.inverse_player_lookup["next_next"]],
            (1825, 250, 30, 30),
        )
        self.screen.blit(
    ...
    def render_action_menu(self):
        ...
        for card in [
            ...
            self.screen.blit(
                self.development_card_images[card], (card_pos[0], card_pos[1])
            )
    ...
    def render_top_menu(self):
        ...
        wood_count = self.count_font.render(
            "x" + str(self.game.resource_bank[Resource.Wood]), False, (255, 255, 255)
        )
        self.screen.blit(wood_count, (428, 23))

        brick_count = self.count_font.render(
            "x" + str(self.game.resource_bank[Resource.Brick]), False, (255, 255, 255)
        )
        self.screen.blit(brick_count, (599, 23))

        sheep_count = self.count_font.render(
            "x" + str(self.game.resource_bank[Resource.Sheep]), False, (255, 255, 255)
        )
        self.screen.blit(sheep_count, (770, 23))

        wheat_count = self.count_font.render(
            "x" + str(self.game.resource_bank[Resource.Wheat]), False, (255, 255, 255)
        )
        self.screen.blit(wheat_count, (940, 23))

        ore_count = self.count_font.render(
            "x" + str(self.game.resource_bank[Resource.Ore]), False, (255, 255, 255)
        )
    ...
    def render_longest_road_largest_army(self):
        ...
        if self.game.largest_army is not None:
            pygame.draw.rect(
                self.screen,
                self.road_colours[self.game.largest_army["player"]],
                (130, 187, 20, 20),
            )
            army_count = self.game.largest_army["count"]
            count_text = self.count_font.render(
                "(" + str(army_count) + ")", False, (0, 0, 0)
            )
        ...
        if self.game.longest_road is not None:
            pygame.draw.rect(
                self.screen,
                self.road_colours[self.game.longest_road["player"]],
                (130, 220, 20, 20),
            )
            road_count = self.game.longest_road["count"]
            count_text_2 = self.count_font.render(
                "(" + str(road_count) + ")", False, (0, 0, 0)
            )
            self.screen.blit(count_text_2, (160, 218))

    def render_trading(self):
        self.screen.blit(
            self.trading_semi_circle_images[self.game.players_go], (1401, 708)
        )
        ...
        for player_id in [PlayerId.White, PlayerId.Red, PlayerId.Blue, PlayerId.Orange]:
            ...
            if (
                ...
                and len(self.active_other_player) > 0
            ):
    ...
    def render_harbours(self):
        for i, harbour in enumerate(self.game.board.harbours):
            ...
            self.screen.blit(
                ...
                (
                    harbour_pos[0] - self.token_dim / 2.0,
    ...
    def render_harbour_res_boxes(self):
        ...
        if len(self.active_harbour) > 0:
            pygame.draw.circle(
                ...
                self.harbour_select_circles[self.active_harbour[0]][1],
                width=4,
            )
        ...
        for player_id in [PlayerId.White, PlayerId.Blue, PlayerId.Red, PlayerId.Orange]:
            if isinstance(self.policies[player_id], str):
                ...
                self.curr_hidden_states[player_id] = (
                    torch.zeros(
                        1,
                        self.dummy_policy.lstm_size,
                        device=self.dummy_policy.dummy_param.device,
                    ),
                    torch.zeros(
                        1,
                        self.dummy_policy.lstm_size,
    ...
    def step_AI(self, deterministic=True):
        ...
        if isinstance(self.policies[players_go], str):
            ...
            action_masks = self.dummy_policy.act_masks_to_torch(
                self.env.get_action_masks()
            )
            ...
            if self.policies[players_go].policy_type == "neural_network":
                ...
                if self.env.game.initial_settlements_placed[players_go] == 0:
                    ...
                    self.env.game.initial_settlements_placed[players_go] == 1
                    and self.env.game.initial_roads_placed[players_go] == 1
                ):
                    placing_initial_settlement = True
            ...
            if (
                ...
                if mouse_click:
                    if len(self.active_development_res_boxes) == 1:
                        ...
                        messagebox.showinfo(
                            "Error", "No resources selected for year of plenty card"
                        )
                        continue
                    action = {
                        "type": ActionTypes.PlayDevelopmentCard,
                ...
                if mouse_click:
                    if len(self.active_development_res_boxes) == 0:
                        ...
                        messagebox.showinfo(
                            "Error", "Can only choose one resource with monopoly."
                        )
                        continue
                    else:
                ...
                if mouse_click:
                    if self.game.players_need_to_discard == False:
                        messagebox.showinfo(
                            "Error", "No one needs to discard resources at the moment"
                        )
                    else:
                        action = {
                ...
                if mouse_click:
                    if len(self.active_other_player) == 0:
                        messagebox.showinfo(
                            "Error", "You need to select a player to steal from"
                        )
                    else:
                        action = {
                ...
                if mouse_click:
                    if self.game.must_respond_to_trade:
                        ...
                        if len(self.active_trade_res) == 0:
                            ...
                            messagebox.showinfo(
                                "Error", "Must choose a player to propose the trade to."
                            )
