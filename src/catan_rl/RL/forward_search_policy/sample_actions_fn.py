...
def default_sample_actions(
    ...
    consider_all_initial_settlements=False,
    initial_settlement_phase=False,
):
    ...
    if type_masks[5] == 1:  # exchange res
        actions_available_type["exchange_res"] = (
            np.sum(action_masks[10]) * np.sum(action_masks[9][0]) - 1
        )
    ...
    if initial_settlement_phase:
        if consider_all_initial_settlements:
            ...
            num_actions_to_sample = int(
                min(max_actions - actions_sampled, effective_actions_available)
            )
    else:
        num_actions_to_sample = int(
            min(max_actions - actions_sampled, effective_actions_available)
    ...
    for i in range(num_actions_to_sample):
        ...
        for j in range(num_priorities):
            avail_types = [
                ac_type
                for ac_type in action_type_priorities[j + 1]
                if actions_available_type.get(ac_type, 0) > 0
            ]
            if (
                trades_proposed >= max_prop_trade_actions
                and "prop_trade" in avail_types
            ):
        ...
        with torch.no_grad():
            _, policy_action, _, next_hs = policy.act(
                obs,
                hidden_state,
                terminal_mask,
                action_masks_torch,
                deterministic=False,
                condition_on_action_type=type_to_ind[ac_type],
            )
