...
from catan_env.game.components.buildings import Building, BuildingType
from catan_env.game.components.corner import Corner
from catan_env.game.components.edge import Edge
from catan_env.game.components.harbour import Harbour
from catan_env.game.components.tile import Tile
...
class Board(object):
    def __init__(
        self,
        randomise_number_placement=True,
        fixed_terrain_placements=None,
        fixed_number_order=None,
    ):
        self.DEFAULT_NUMBER_ORDER = [
            5,
            2,
            6,
            3,
        ...
        self.HARBOURS_TO_PLACE = [
            Harbour(Resource.Ore, exchange_value=2, id=0),
            Harbour(Resource.Sheep, exchange_value=2, id=1),
            Harbour(Resource.Wheat, exchange_value=2, id=2),
            Harbour(Resource.Wood, exchange_value=2, id=3),
            Harbour(Resource.Brick, exchange_value=2, id=4),
            Harbour(None, exchange_value=3, id=5),
            Harbour(None, exchange_value=3, id=6),
            Harbour(None, exchange_value=3, id=7),
        ...
        if fixed_terrain_placements is not None:
            assert np.array_equal(
                [
                    fixed_terrain_placements.count(terrain)
                    == self.TERRAIN_TO_PLACE.count(terrain)
                    for terrain in [
                        Terrain.Hills,
                        Terrain.Forest,
                        Terrain.Fields,
        ...
        if fixed_number_order is not None:
            assert np.array_equal(
                [
                    fixed_number_order.count(n)
                    == self.DEFAULT_NUMBER_ORDER.count(n)
                    for n in range(2, 13)
                ]
            )
        ...
        for i in range(19):
            if tile_vals[i] == 6 or tile_vals[i] == 8:
                for key in TILE_NEIGHBOURS[i]:
                    ...
                    if (
                        tile_vals[neighbour_ind] == 6
                        or tile_vals[neighbour_ind] == 8
                    ):
                        return False
        return True
    ...
    def reset(self):
        ...
        if self.fixed_number_order is not None:
            ...
            if self.randomise_number_placement:
                ...
                while (
                    self.validate_number_order(number_order, terrain_order)
                    == False
                ):
                    np.random.shuffle(number_order)
        ...
        for i in range(19):
            if (
                self.tiles[self.NUMBER_PLACEMENT_INDS[i]].terrain
                == Terrain.Desert
            ):
                self.tiles[self.NUMBER_PLACEMENT_INDS[i]].value = 7
                self.tiles[self.NUMBER_PLACEMENT_INDS[i]].contains_robber = (
                    True
                )
                self.robber_tile = self.tiles[self.NUMBER_PLACEMENT_INDS[i]]
            else:
                self.tiles[self.NUMBER_PLACEMENT_INDS[i]].value = number_order[
                    num_ind
                ]
                self.value_to_tiles[number_order[num_ind]] = (
                    self.value_to_tiles.get(number_order[num_ind], [])
        ...
        for tile_ind in range(19):
            for corner_location in self.tiles[tile_ind].corners.keys():
                ...
                if prev_tile_ind is None:
                    self.tiles[tile_ind].corners[corner_location] = (
                        self.corners[corner_ind]
                    )
                    corner_ind += 1
                else:
                    self.tiles[tile_ind].corners[corner_location] = self.tiles[
                        prev_tile_ind
                    ].corners[prev_corner_loc]
            ...
            for edge_location in self.tiles[tile_ind].edges.keys():
                ...
                if prev_tile_ind is None:
                    self.tiles[tile_ind].edges[edge_location] = self.edges[
                        edge_ind
                    ]
                    edge_ind += 1
                else:
                    self.tiles[tile_ind].edges[edge_location] = self.tiles[
                        prev_tile_ind
                    ].edges[prev_edge_loc]

        for tile_ind in range(19):
            for corner_loc, corner in self.tiles[tile_ind].corners.items():
                for n_corner_loc in CORNER_NEIGHBOURS_IN_TILE[
                    corner_loc
                ].keys():
                    edge_loc = CORNER_NEIGHBOURS_IN_TILE[corner_loc][
                        n_corner_loc
                    ]
        ...
        for corner in self.corners:
            for n_corner in corner.corner_neighbours:
                if n_corner[0] is not None:
                    self.corner_adjacency_matrix[corner.id, n_corner[0].id] = (
                        1.0
                    )
                    self.corner_egde_identification_map[
                        corner.id, n_corner[0].id
                    ] = n_corner[1].id

    def insert_settlement(self, player, corner, initial_placement=False):
        if corner.can_place_settlement(
            player.id, initial_placement=initial_placement
        ):
    ...
    def insert_city(self, player, corner):
        if corner.building is not None and (
            corner.building.type == BuildingType.Settlement
    ...
    def get_available_settlement_locations(self, player, initial_round=False):
        ...
        if initial_round == False:
            if (
                ...
                and player.resources[Resource.Wheat] > 0
            ):
                pass
            else:
        ...
        for i, corner in self.corners:
            if corner.can_place_settlement(
                player.id, initial_placement=initial_round
            ):
    ...
    def get_available_city_locations(self, player):
        ...
        if (
            ...
            and player.resources[Resource.Wheat] >= 2
        ):
            pass
        else:
            return available_locations
