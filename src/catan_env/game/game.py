...
from catan_env.game.components.board import Board
...
from catan_env.game.utils import DFS
...
class Game():
    ...
    def roll_dice(self):
            ...
            for corner_key, corner in tile.corners.items():
                if corner.building is not None:
                    if corner.building.type == BuildingType.Settlement:
                        ...
                        increment = 2
                    resources_allocated[tile.resource][
                        corner.building.owner
                    ] += increment
        ...
        for resource in resources_allocated.keys():
            if (
                resources_allocated[resource]["total"]
                <= self.resource_bank[resource]
            ):
                for player in [
                    PlayerId.Blue,
                    PlayerId.Orange,
                    ...
                    self.players[player].resources[
                        resource
                    ] += resources_allocated[resource][player]
                    self.resource_bank[resource] -= resources_allocated[
                        resource
                    ][player]
                    self.update_player_resource_estimates(
                        {resource: resources_allocated[resource][player]},
                        player,
                    )
    ...
    def can_buy_development_card(self, player):
        if (
            player.resources[Resource.Wheat] > 0
            and player.resources[Resource.Sheep] > 0
            and player.resources[Resource.Ore] > 0
        ):
            return True
        else:
            return False
    ...
    def can_buy_settlement(self, player):
        ...
        if self.building_bank["settlements"][player.id] > 0:
            if (
                player.resources[Resource.Wheat] > 0
                and player.resources[Resource.Wood] > 0
    ...
    def build_settlement(self, player, corner):
        if self.initial_placement_phase == False:
            ...
            player.visible_resources[Resource.Wheat] = max(
                player.visible_resources[Resource.Wheat] - 1, 0
            )
            player.visible_resources[Resource.Sheep] = max(
                player.visible_resources[Resource.Sheep] - 1, 0
            )
            player.visible_resources[Resource.Wood] = max(
                player.visible_resources[Resource.Wood] - 1, 0
            )
            player.visible_resources[Resource.Brick] = max(
                player.visible_resources[Resource.Brick] - 1, 0
            )
        ...
        self.board.insert_settlement(
            player, corner, initial_placement=self.initial_placement_phase
        )
    ...
    def can_buy_city(self, player):
        if self.building_bank["cities"][player.id] > 0:
            if (
                player.resources[Resource.Wheat] > 1
                and player.resources[Resource.Ore] > 2
            ):
                return True
        return False
    ...
    def build_city(self, player, corner):
        ...
        player.visible_resources[Resource.Wheat] = max(
            player.visible_resources[Resource.Wheat] - 2, 0
        )
        player.visible_resources[Resource.Ore] = max(
            player.visible_resources[Resource.Ore] - 3, 0
        )
    ...
    def validate_action(self, action, check_player=False):
        if check_player and self.policies is not None:
            if self.players_need_to_discard:
                ...
                curr_player = self.players[
                    self.proposed_trade["target_player"]
                ].id
            else:
                curr_player = self.players[self.players_go].id
            if (
                isinstance(self.policies[curr_player], str)
                and self.policies[curr_player] == "human"
            ):
                pass
            else:
                return (
                    False,
                    "This player is registered as an AI. Click AI Decision to make them play.",
                )
        ...
        if self.players_need_to_discard:
            if action["type"] != ActionTypes.DiscardResource:
                return (
                    False,
                    "A 7 was rolled and you have more than 7 resources. You must discard resources until you only have 7 resources.",
                )
            else:
                ...
                if total_current_res <= 7:
                    raise ValueError(
                        "Player is being told to discard when they have less than 7 or less resources!"
                    )
                else:
                    ...
                    if total_current_res - len(action["resources"]) < 7:
                        return (
                            False,
                            "You are trying to discard too many resources!",
                        )
                    ...
                    for res in action["resources"]:
                        if curr_res[res] <= 0:
                            return (
                                False,
                                "You do not have these resources to discard!",
                            )
                        else:
                            curr_res[res] -= 1
                    return True, None
        ...
        if action["type"] == ActionTypes.PlaceSettlement:
            ...
            if (
                self.dice_rolled_this_turn == False
                and self.initial_placement_phase == False
            ):
                return False, "You need to roll the dice first!"
            elif self.must_use_development_card_ability:
                return (
                    False,
                    "You need to play out your development card ability first.",
                )
            elif self.just_moved_robber:
                return (
                    False,
                    "You've just moved the robber. Choose a player to steal from first.",
                )
            if self.can_buy_settlement(player):
                corner = action["corner"]
                if self.board.corners[corner].can_place_settlement(
                    player.id, initial_placement=self.initial_placement_phase
                ):
                    if self.initial_placement_phase:
                        if self.initial_settlements_placed[
                            self.players_go
                        ] == 0 or (
                            self.initial_settlements_placed[self.players_go]
                            == 1
                            and self.initial_roads_placed[self.players_go] == 1
                        ):
                            return True, None
                ...
                if self.board.edges[edge].can_place_road(player.id):
                    if self.initial_placement_phase:
                        if (
                            self.initial_settlements_placed[self.players_go]
                            == 1
                            and self.initial_roads_placed[self.players_go] == 0
                        ):
                            return True, None
                        elif (
                            self.initial_settlements_placed[self.players_go]
                            ...
                            if self.board.edges[edge].can_place_road(
                                player.id,
                                after_second_settlement=True,
                                second_settlement=self.initial_second_settlement_corners[
                                    player.id
                                ],
                            ):
                                return True, None
                                ...
                                return (
                                    False,
                                    "Must place second road next to second settlement.",
                                )
            ...
            if self.initial_placement_phase:
                return (
                    False,
                    "Still in initial placement phase. Cannot buy a development card",
                )
            elif self.dice_rolled_this_turn == False:
                return False, "You need to roll the dice first!"
            elif self.must_use_development_card_ability:
            ...
            if self.played_development_card_this_turn:
                return (
                    False,
                    "You have already played one development card this turn.",
                )
            ...
            if action["card"] in player.hidden_cards:
                if player.hidden_cards.count(action["card"]) > 0:
                    if player.hidden_cards.count(
                        action["card"]
                    ) == self.development_cards_bought_this_turn.count(
                        action["card"]
                    ):
                        return (
                            False,
                            "Cannot play a card the same turn as it was bought.",
                        )
            ...
            if player.resources[trading_resource] >= trade_ratio:
                if self.resource_bank[target_resource] > 0:
                    ...
                    return (
                        False,
                        "Not enough of desired resource left in the bank.",
                    )
            else:
                return (
                    False,
                    "You do not have enough resources for this exchange.",
                )
            ...
            if self.initial_placement_phase:
                ...
                return (
                    False,
                    "You cannot end your turn before rolling the dice!",
                )
            elif self.must_use_development_card_ability:
                return (
                    False,
            ...
            if self.just_moved_robber:
                ...
                return (
                    False,
                    "Cannot steal from a player who doesn't have a building on tile with robber.",
                )
            else:
                return (
                    False,
                    "You cannot steal a resource when you haven't moved the robber.",
                )
    ...
    def apply_action(self, action, return_message=True):
        ...
        if action["type"] == ActionTypes.PlaceSettlement:
            ...
            if self.initial_placement_phase:
                ...
                if self.initial_settlements_placed[player.id] == 2:
                    ...
                    self.initial_second_settlement_corners[player.id] = action[
                        "corner"
                    ]
            else:
                building_res = {
            ...
            if edge is None:
                message = {
                    "player_id": player.id,
                    "text": "Placed dummy road (nothing happened).",
                }
            else:
                self.build_road(
                    player, edge, road_building=self.road_building_active[0]
                )
                if self.initial_placement_phase:
                    ...
                    for (
                        player_id,
                        count,
                    ) in self.initial_settlements_placed.items():
                        if count == 1:
                    ...
                    if first_settlements_placed < 4:
                        ...
                        first_settlements_placed == 4
                        and second_settlements_placed == 0
                    ):
                        pass
                    elif (
                        first_settlements_placed == 4
                        and second_settlements_placed < 4
                    ):
            ...
            if self.road_building_active[0]:
                ...
                if (
                    self.initial_placement_phase == False
                    and placing_final_init_road == False
                ):
                    self.update_player_resource_estimates(
            ...
            if return_message:
                message = {
                    "player_id": player.id,
                    "text": "Upgraded their settlement into a city.",
                }
            self.update_player_resource_estimates(
            ...
            if return_message:
                message = {
                    "player_id": player.id,
                    "text": "Rolled the dice (" + str(roll_value) + ").",
                }
        elif action["type"] == ActionTypes.EndTurn:
            self.can_move_robber = False
            self.dice_rolled_this_turn = False
            ...
            self.can_move_robber = False
            tile_corner_values = self.board.tiles[
                action["tile"]
            ].corners.values()
            player_to_steal_from = False
            for value in tile_corner_values:
                if (
                    value.building is not None
                    and value.building.owner != player.id
                ):
            ...
            for res in [
                ...
                resources_to_steal += [res] * self.players[
                    player_id
                ].resources[res]
            if len(resources_to_steal) == 0:
                ...
                for res in [
                    ...
                    self.players[player_id].visible_resources[res] = max(
                        self.players[player_id].visible_resources[res] - 1, 0
                    )
                self.update_player_resource_estimates(
                    {resource_to_steal: -1}, player_id, player.id
                )
            self.just_moved_robber = False
            if return_message:
                message = {"player_id": player.id}
                message["text"] = (
                    "Stole a resource from {style}{color "
                    + str(self.colours[action["target"]])
                    + "}Player"
                )
        elif action["type"] == ActionTypes.PlayDevelopmentCard:
            ...
            if action["card"] == DevelopmentCard.VictoryPoint:
                ...
                card_type_text = (
                    "Monopoly card, taking "
                    + self.resource_text[resource]
                    + "."
                )
                num_res = {}
                ...
                self.update_resource_estimates_monopoly(
                    player.id, resource, num_res
                )
                ...
                for resource in [action["resource_1"], action["resource_2"]]:
                    if self.resource_bank[resource] > 0:
                        ...
                        self.update_player_resource_estimates(
                            {resource: 1}, player.id
                        )
                card_type_text = (
                    "Year of Plenty card, collecting {} and {}.".format(
                        self.resource_text[action["resource_1"]],
            ...
            player.visible_resources[Resource.Ore] = max(
                player.visible_resources[Resource.Ore] - 1, 0
            )
            player.visible_resources[Resource.Wheat] = max(
                player.visible_resources[Resource.Wheat] - 1, 0
            )
            ...
            self.resource_bank[Resource.Wheat] += 1
            self.update_player_resource_estimates(
                {Resource.Sheep: -1, Resource.Ore: -1, Resource.Wheat: -1},
                player.id,
            )
            player.hidden_cards.append(self.development_cards_pile.pop())
            self.development_cards_bought_this_turn.append(
                copy.copy(player.hidden_cards[-1])
            )
            if return_message:
                message = {
                    "player_id": player.id,
                    "text": "Bought a development card.",
                }
        elif action["type"] == ActionTypes.ExchangeResource:
            ...
            player.visible_resources[trade_resource] = max(
                player.visible_resources[trade_resource]
            ...
            self.update_player_resource_estimates(res_dict, player.id)
            if return_message:
                message = {
                    "player_id": player.id,
                    "text": "Exchanged {} {} for {}".format(
                        action["exchange_rate"],
            ...
            if return_message:
                message = {"player_id": player.id}
                text = (
                    "Proposed a deal to {style}{color "
                    + str(self.colours[action["target_player"]])
                    + "}Player, {style}{color "
                    + str(self.colours[player.id])
                    + "}offering "
                )
            ...
            if action["response"] == "accept":
                ...
                for res in trade["player_proposing_res"]:
                    ...
                    self.players[p1].visible_resources[res] = max(
                        self.players[p1].visible_resources[res] - 1, 0
                    )
                    res_p1[res] -= 1
                ...
                for res in trade["target_player_res"]:
                    ...
                    res_p1[res] += 1
                    self.players[p2].resources[res] -= 1
                    self.players[p2].visible_resources[res] = max(
                        self.players[p2].visible_resources[res] - 1, 0
                    )
                    res_p2[res] -= 1
        ...
        if (
            action["type"] != ActionTypes.RespondToOffer
            and action["type"] != ActionTypes.EndTurn
            and action["type"] != ActionTypes.DiscardResource
        ):
    ...
    def update_largest_army(self):
        ...
        for player_id in [
            ...
            knight_count = self.players[player_id].visible_cards.count(
        ...
        if count_player is not None:
            largest_army_update = {"player": count_player, "count": max_count}
    ...
    def get_longest_path(self, player_id):
        ...
        for s, t in player_edges:
            if (
                self.board.corners[s].building is not None
                and self.board.corners[s].building.owner != player_id
            ):
                pass
            else:
                G[s].append(t)
            if (
                self.board.corners[t].building is not None
                and self.board.corners[t].building.owner != player_id
            ):
                pass
            else:
                G[t].append(s)
    ...
    def update_longest_road(self, player_id):
        ...
        longest_road_update = {"player": player_id, "count": max_path_len}
    ...
    def update_player_resource_estimates(
        self, resources, player_updating, player_who_stole=None
    ):
        total_resources = sum(self.players[player_updating].resources.values())
        if player_who_stole is not None:
            total_resources_stealer = sum(
                self.players[player_who_stole].resources.values()
            )
        for player in [
            PlayerId.White,
            PlayerId.Red,
            ...
            if player == player_updating:
                if player_who_stole is None:
                    continue
                stealing_player_label = self.players[player].player_lookup[
                    player_who_stole
                ]
                for res, num_res in resources.items():
                    # player knows what was stolen
                    self.players[player].opponent_max_res[
                        stealing_player_label
                    ][res] -= num_res
                    self.players[player].opponent_min_res[
                        stealing_player_label
                    ][res] -= num_res
            else:
                player_label = self.players[player].player_lookup[
                    player_updating
                ]
                if player_who_stole is None:
                    # visible exchange of resources
                    for res, num_res in resources.items():
                        curr_max_est = self.players[player].opponent_max_res[
                            player_label
                        ][res]
                        curr_min_est = self.players[player].opponent_min_res[
                            player_label
                        ][res]
                        self.players[player].opponent_max_res[player_label][
                            res
                        ] = np.clip(curr_max_est + num_res, 0, total_resources)
                        self.players[player].opponent_min_res[player_label][
                            res
                        ] = np.clip(curr_min_est + num_res, 0, total_resources)
                else:
                    if player == player_who_stole:
                        # knows exactly what was stolen
                        for res, num_res in resources.items():
                            curr_max_est = self.players[
                                player
                            ].opponent_max_res[player_label][res]
                            curr_min_est = self.players[
                                player
                            ].opponent_min_res[player_label][res]
                            self.players[player].opponent_max_res[
                                player_label
                            ][res] = np.clip(
                                curr_max_est + num_res, 0, total_resources
                            )
                            self.players[player].opponent_min_res[
                                player_label
                            ][res] = np.clip(
                                curr_min_est + num_res, 0, total_resources
                            )
                    else:
                        stealing_player_label = self.players[
                            player
                        ].player_lookup[player_who_stole]
                        for res in [
                            Resource.Wood,
                            Resource.Brick,
                            Resource.Wheat,
                            ...
                            curr_max_player_est = self.players[
                                player
                            ].opponent_max_res[player_label][res]
                            curr_min_player_est = self.players[
                                player
                            ].opponent_min_res[player_label][res]
                            self.players[player].opponent_max_res[
                                player_label
                            ][res] = np.clip(
                                curr_max_player_est, 0, total_resources
                            )
                            # any resource could have been stolen
                            self.players[player].opponent_min_res[
                                player_label
                            ][res] = np.clip(
                                curr_min_player_est - 1, 0, total_resources
                            )
                            if curr_max_player_est > 0:
                                curr_max_steal_est = self.players[
                                    player
                                ].opponent_max_res[stealing_player_label][res]
                                curr_min_steal_est = self.players[
                                    player
                                ].opponent_min_res[stealing_player_label][res]
                                self.players[player].opponent_max_res[
                                    stealing_player_label
                                ][res] = np.clip(
                                    curr_max_steal_est + 1,
                                    0,
                                    total_resources_stealer,
                                )
                                self.players[player].opponent_min_res[
                                    stealing_player_label
                                ][res] = np.clip(
        ...
        for player in [
            ...
            if player == player_id:
                ...
                for o_player in [
                    ...
                    self.players[o_player].opponent_min_res[player_label][
                        res
                    ] += total_res
                    self.players[o_player].opponent_max_res[player_label][
                        res
                    ] += total_res
            else:
                ...
                for o_player in [
                    ...
                    for o_res in [
                        ...
                        curr_max_est = self.players[o_player].opponent_max_res[
                            player_label
                        ][o_res]
                        curr_min_est = self.players[o_player].opponent_min_res[
                            player_label
                        ][o_res]
                        if o_res == res:
                        ...
                        self.players[o_player].opponent_max_res[player_label][
                            o_res
                        ] = np.clip(curr_max_est, 0, player_total_res)
                        self.players[o_player].opponent_min_res[player_label][
                            o_res
                        ] = np.clip(curr_min_est, 0, player_total_res)
    ...
    def save_current_state(self):
        ...
        for corner in self.board.corners:
            if corner.building is not None:
                state["corner_buildings"].append(
                    (corner.building.type, corner.building.owner)
                )
            else:
                state["corner_buildings"].append((None, None))

        state["harbour_order"] = [
            harbour.id for harbour in self.board.harbours
        ]
        ...
        for player_key, player in self.players.items():
            state["players"][player_key] = {
                ...
                "inverse_player_lookup": copy.copy(
                    player.inverse_player_lookup
                ),
                ...
                "harbour_info": [
                    (key, val.id) for key, val in player.harbours.items()
                ],
        ...
        state["development_card_pile"] = copy.deepcopy(
            self.development_cards_pile
        )
        ...
        state["initial_settlements_placed"] = copy.copy(
            self.initial_settlements_placed
        )
        state["initial_roads_placed"] = copy.copy(self.initial_roads_placed)
        state["initial_second_settlement_corners"] = copy.copy(
            self.initial_second_settlement_corners
        )

        state["dice_rolled_this_turn"] = self.dice_rolled_this_turn
        state["played_development_card_this_turn"] = (
            self.played_development_card_this_turn
        )
        state["must_use_development_card_ability"] = (
            self.must_use_development_card_ability
        )
        ...
        state["development_cards_bought_this_turn"] = (
            self.development_cards_bought_this_turn
        )
    ...
    def restore_state(self, state):
        ...
        for i, info in enumerate(state["tile_info"]):
            terrain, resource, value, likelihood, contains_robber = (
                info[0],
                info[1],
                info[2],
                info[3],
            ...
            if value != 7:
                if value in self.board.value_to_tiles:
                    self.board.value_to_tiles[value].append(
                        self.board.tiles[i]
                    )
                else:
        ...
        for i, entry in enumerate(state["corner_buildings"]):
            ...
            if building is not None:
                self.board.corners[i].building = Building(
                    building, player, self.board.corners[i]
                )
            else:
                self.board.corners[i].building = None

        """have to reinitialise harbours - annoying"""
        self.board.harbours = copy.copy(
            [self.board.HARBOURS_TO_PLACE[i] for i in state["harbour_order"]]
        )
        ...
        for key, player_state in state["players"].items():
            ...
            self.players[key].inverse_player_lookup = player_state[
                "inverse_player_lookup"
            ]
            ...
            self.players[key].visible_resources = player_state[
                "visible_resources"
            ]
            self.players[key].opponent_max_res = player_state[
                "opponent_max_res"
            ]
            self.players[key].opponent_min_res = player_state[
                "opponent_min_res"
            ]
        ...
        self.initial_second_settlement_corners = state[
            "initial_second_settlement_corners"
        ]

        self.dice_rolled_this_turn = state["dice_rolled_this_turn"]
        self.played_development_card_this_turn = state[
            "played_development_card_this_turn"
        ]
        self.must_use_development_card_ability = state[
            "must_use_development_card_ability"
        ]
        ...
        self.development_cards_bought_this_turn = state[
            "development_cards_bought_this_turn"
        ]
    ...
    def randomise_uncertainty(self, controlling_player_id):
        ...
        """dev cards"""
        all_possible_development_cards = list(
            copy.copy(self.development_cards_pile)
        )
        for player_id in self.players.keys():
            if player_id != controlling_player_id:
                all_possible_development_cards += list(
                    copy.copy(self.players[player_id].hidden_cards)
                )
        ...
        for player_id in self.players.keys():
            if player_id != controlling_player_id:
                ...
                self.players[player_id].hidden_cards = [
                    all_possible_development_cards.pop()
                    for _ in range(num_hidden_cards)
                ]
        ...
        for player_id in self.players.keys():
            total_resources_before[player_id] = sum(
                self.players[player_id].resources.values()
            )
        ...
        for res in [
            ...
            for player_id in self.players.keys():
                if player_id != controlling_player_id:
                    player_definite_res = self.players[
                        controlling_player_id
                    ].opponent_min_res[
                        self.players[controlling_player_id].player_lookup[
                            player_id
                        ]
                    ][
                        res
                    ...
                    res_accounted_for[res] += self.players[
                        player_id
                    ].resources[res]
        ...
        while consistent_distribution_reached == False:
            proposed_res = {
                player_id: copy.deepcopy(self.players[player_id].resources)
                for player_id in self.players.keys()
            }
            res_list = []
                ...
                for player_id in player_keys:
                    if player_id != controlling_player_id:
                        ...
                        if total_resources < total_resources_before[player_id]:
                            ...
                            if (
                                max_res
                                > proposed_res[player_id][res_to_distribute]
                            ):
        ...
        for player_id in self.players.keys():
            self.players[player_id].resources = copy.copy(
                proposed_res[player_id]
            )
