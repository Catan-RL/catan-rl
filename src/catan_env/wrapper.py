...
class EnvWrapper(object):
    ...
    def _get_obs(self):
        ...
        for target_p in ["next", "next_next", "next_next_next"]:
            obs[target_p + "_player_main"], obs[target_p + "_player_played_dev"], _ = (
                self._get_player_inputs(player, target_p)
            )
    ...
    def _translate_action(self, action):
        ...
        if (
            ...
            translated_action["target"] = self.game.players[
                players_go
            ].inverse_player_lookup[target_id]
    ...
    def get_action_masks(self):
        ...
        if self.game.initial_placement_phase:
            if (
                self.game.initial_settlements_placed[player.id] == 0
                or self.game.initial_settlements_placed[player.id] == 1
        ...
        if self.game.road_building_active[0]:
            ...
            if (
                ...
                and self.game.played_development_card_this_turn == False
            ):
        ...
        if self.max_proposed_trades_per_turn is None:
            ...
            if (
                self.game.trades_proposed_this_turn < self.max_proposed_trades_per_turn
                and total_res > 0
            ):
                valid_actions[0][ActionTypes.ProposeTrade] = 1.0
        return valid_actions
    ...
    def _get_valid_robber_locations(self):
        ...
        for i, tile in enumerate(self.game.board.tiles):
            ...
            for key in tile.corners.keys():
                if (
                    tile.corners[key].building is not None
                    and tile.corners[key].building != curr_player
                ):
                    valid = True
                    break
            if valid:
    ...
    def _get_valid_actions_play_dev_card(self, player):
        ...
        for card in [
            ...
            if card_count > 0:
                if (
                    self.game.development_cards_bought_this_turn.count(card)
                    < card_count
                ):
            ...
            for corner_id in ["T", "TL", "BL", "B", "BR", "TR"]:
                ...
                if corner.building is not None:
                    ...
                    if building_owner == player.id:
                        ...
                        building_owner == player.inverse_player_lookup["next_next_next"]
                    ):
                        owner[3] = 1.0
        ...
        if self.game.longest_road is not None:
            if self.game.longest_road["player"] == target_player.id:
                ...
                if target_player.id in self.game.current_longest_path:
                    longest_road[1] = (
                        self.game.current_longest_path[target_player.id] / 8.0
                    )
